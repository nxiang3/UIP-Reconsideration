...
vec_gamma0twid = inv(eye(nztwid^2)-kron(phitwid,phitwid))*omegaUtwid(:);
gamma0twid = reshape(vec_gamma0twid,nztwid,[]);
vec_gamma0 = inv(eye(nz^2)-kron(phi,phi))*omegaU(:);
...
if iflag==0
    ...
    vbias1 = zeros(nk,1)-inv(bigD)*PX*inv(eye(nz)-phi)*omegaU*vec_delta;
else
    temp = Eetaztwid(:,:,nq+1)';
	vbias1 = zeros(nk,1)-inv(bigD)*(EXeta+PX*inv(eye(nz)-phi)*temp(:));
end
...
for r=1:nk
	...
	if iflag == 0
		...
		vec_Frtwid = inv(eye(nztwid^2)-kron(phitwid,phitwid))*vec_Frtwid;
		...
		vec_Frbtwid = inv(eye(nztwid^2)-kron(phitwid,phitwid))*vec_Frbtwid;
