...
keep country2 cty2 date year month s_change s_yoy i_sum i_diff_forward* i_diff p_r_diff
...
forval i = 1(1)11{
	...
	* adjusted standard error (Boudoukh, Richardson and  Whitelaw'2008')
	...
	egen i_diff_var = sd(i_diff)
	replace i_diff_var = i_diff_var^2
	g se = sqrt(J*sr_var/T/i_diff_var*sqrt(1 + 2*rho/J/(1-rho)*((J-1) - rho*(1-rho^(J-1))/(1-rho))))
	g high = beta_J + 1.96*se
	g low = beta_J -1.96*se
	g P_value = 2*ttail(e(df_r), abs((beta_J)/se))
	keep country2 T beta_J se high low P_value
	duplicates drop 
	save $inter/mr_`i'_`x', replace
	restore
}

use $inter/mr_1_`x', clear
forval i = 2(1)11{
    append using $inter/mr_`i'_`x'
}
	...
	save $results/corrected_mrfama_`x', replace
}
