******************************************************
*    Bias Correction for Restricted Inflation Fama   *
*                By: Nan Xiang                       *
******************************************************
...
foreach x of global date{
	...
	gen i_diff_rest = i_diff - p_r_diff
	gen i_diff_rest_lag = l.i_diff_rest
	...
	gen P_value =.
forval i = 1(1)9{
    reg i_diff_rest i_diff_rest_lag if cty2 == `i', r
	replace var_rho = _se[i_diff_rest_lag]^2 if cty2 == `i'
	* Step 1: construct corrected residuals
	replace rho_c = _b[i_diff_rest_lag] + (1+3 * _b[i_diff_rest_lag])/num + 3*(1+3 * _b[i_diff_rest_lag])/num/num if cty2 == `i'
	replace theta_c = _b[_cons] if cty2 == `i'
	replace v_c = i_diff_rest - rho_c*i_diff_rest_lag - theta_c if cty2 == `i'
	xtset cty2 t
	replace v_c = f.v_c if cty2 == `i'
	* Step 2: estimate fama coefficient
	reg srex i_diff_rest v_c if cty2 == `i',r
	replace beta_c = _b[i_diff_rest] if cty2 == `i'
	replace phi_c = _b[v_c] if cty2 == `i'
	* Step 3: SE correction
	replace var_rho_c = var_rho*(1+3/num + 9/num/num)^2 if cty2 == `i'
	replace var_beta_c = _se[i_diff_rest]^2 if cty2 == `i'
	...
	save $results/corrected_restricted_fama_pi_`x', replace
}	
