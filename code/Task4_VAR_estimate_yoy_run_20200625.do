...
foreach country in CAD CHF DEM FRF GBP ITL JPY NOK SEK{
	...
	* deviation from mean of real exchange rate, relative inflation and relative interest rate
	qui: var q pi_ryoy i_diff, lags(1/3)
	* Residuals Prediction
	...
	gen x_T = p_r - (p_r + p_r_lag1 + p_r_lag2 + p_r_lag3 + p_r_lag4 + p_r_lag5 + p_r_lag6 + p_r_lag7 + p_r_lag8 + p_r_lag9 + p_r_lag10 + p_r_lag11) / 12
	...
	qui: forval i = 1/`=N'{
			...
			replace s_T = q_dev - temp2[1,1] + x_T if t == `i'
		}
	gen s_diff = s_T - s_IP
...
//normalized_idiff
	...
	replace cihigh_t = `cihigh'
	outreg2 using "temp/s_t_3lag_norm.xls", append dec(3) stats(coef se) adds(t-test, `tstat', CI-low, `cilow', CI-high, `cihigh')

	newey s_IP norm_idiff, lag(3)
	replace coef_ip = _b[norm_idiff] 
	local tstat = _b[norm_idiff]/_se[norm_idiff]
	replace tstat_ip = `tstat'
	local cilow = _b[norm_idiff] - 1.96 * _se[norm_idiff]
	replace cilow_ip = `cilow' 
	local cihigh = _b[norm_idiff] + 1.96 * _se[norm_idiff]	
	replace cihigh_ip = `cihigh'
	outreg2 using "temp/s_ip_3lag_norm.xls", append dec(3) stats(coef se) adds(t-test, `tstat', CI-low, `cilow', CI-high, `cihigh')
	...
	//Regression excess
	newey s_diff norm_idiff, lag(3)
	replace coef_diff = _b[norm_idiff] 
	local tstat = _b[norm_idiff]/_se[norm_idiff]
	replace tstat_diff = `tstat' 
	local cilow = _b[norm_idiff] - 1.96 * _se[norm_idiff]
	replace cilow_diff = `cilow' 
	local cihigh = _b[norm_idiff] + 1.96 * _se[norm_idiff]	
	replace cihigh_diff = `cihigh'
	outreg2 using "temp/s_diff_3lags_norm.xls", append dec(3) stats(coef se) adds(t-test, `tstat', CI-low, `cilow', CI-high, `cihigh')
